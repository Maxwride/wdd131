...
function renderRecipes(recipeList) {
    let recipeContainer = document.querySelector('#searchResults');
    recipeContainer.innerHTML += recipeTemplate(recipeList); // Append the template
}
...
let button = document.querySelector(".search-button");
	...
	const filterarray = recipes.filter(function(item){
		return(
			item.name.toLowerCase().includes(userinput.toLowerCase()) || 
			item.description.toLowerCase().includes(userinput.toLowerCase()) || 
			item.tags.find((tag)=> tag.toLowerCase().includes(userinput.toLowerCase()))
		)
	})
